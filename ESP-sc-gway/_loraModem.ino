...
void rxLoraModem()
{
	...
	// Set frequency based on value in freq
	setFreq();
...
int sendPacket(uint8_t *buff_down, uint8_t length) {
	...
	const uint8_t psize = root["txpk"]["size"];
	const bool ipol = root["txpk"]["ipol"];
	const uint8_t powe = root["txpk"]["powe"];
	const uint32_t tmst = root["txpk"]["tmst"];
	...
	if (debug >=1) {
		...
		Serial.print(tmst);
		Serial.print(F(", send in: "));
		Serial.print(tmst - trx);				// TimeGap
		Serial.print(F(", Tnow: "));
		Serial.print(micros());
		Serial.println();
	}
